...
/*
 *  LFN
 *  http://www.microchip.com/forums/m456457.aspx
 *
 */
#ifdef LFNREAD// Long file name directory entry structure
typedef struct
{
	BYTE	seqno;
	BYTE	name1[10];
...
typedef _LFNDIRENTRY * LFNDIRENTRY;
#endif//LFNREAD
...
#ifdef LFNREAD
	char LFN[64];	/*(11/04/09) IANC: Store long file names here during Fill_File_Object */
	BYTE LFNokflag;/* Track whether LFN being built is valid or not */
	BYTE LFNstate;/* Track if all data in directory is good for building LFN */
	BYTE LFNchecksum;/* Used to confirm LFN matches following DOS/short file name */
        char *LFNifEpsaen;
#endif//LFNREAD
...
{
    DIRENTRY    dir;
#ifdef LFNREAD//(11/04/09)
    LFNDIRENTRY lfndir;/*LFNREAD*/
#endif
    BYTE        index, a;
    BYTE        character;
    BYTE        status;
    BYTE        test = 0;
    WORD		lfncharacter;/*LFNREAD*/
        ...
        if ( a == DIR_DEL)
	{
        ...
        else
        {
            ...
            if (a==0x0F)
            {
                            lfndir = (LFNDIRENTRY)dir;
                            a = lfndir->seqno;/* Attribute indicates this is LFN data - process it! */
                            if (a&0x40)
                            {

                                    a = a&~0x40;/* This is the end-LFN record - note it's sequence code */
                                    LFNstate = a;
                                    LFNokflag = 0;/* For now, we assume the LFN will be OK */
                                    LFNchecksum = lfndir->checksum;/* Remember the checksum */
                                    for (index=0; index<sizeof(LFN); index++)
                                    {
                                            LFN[index] = 0;/* Wipe the LFN buffer */
                                    }
                            }
                            if (a==LFNstate)
                            {
                                    if (a==1)
                                    {
                                            LFNokflag = LFNokflag|0x01;/* This is the start-LFN record; flag that we've done the LFN parse */
                                    }
                                    LFNstate--;/* Make sure we check next sequence, next time */
                                    a = LFNstate*13;/* Point to where to put the characters from this part of LFN */
                                    for (index=0; index<sizeof(lfndir->name1);)/* Copy the first section of characters */
                                    {
                                            lfncharacter = (((WORD)lfndir->name1[index+1])<<8)+lfndir->name1[index];/* Get next LFN character, and copy to LFN buffer */
                                            if (lfncharacter==0xFFFF)
                                            {
                                                    /* Ignore FFFF padding after end of name */
                                            }
                                            else
                                            {
                                                    if (a<sizeof(LFN))
                                                    {
                                                            LFN[a++] = (char)lfncharacter;
                                                    }
                                                    else
                                                    {
                                                            LFNokflag = LFNokflag|0x20;/* filename too long! */
                                                    }
                                                    if (((lfncharacter>0)&&(lfncharacter<32))||(lfncharacter>127))
                                                    {
                                                            LFNokflag = LFNokflag|0x40;/* We have non-ASCII characters in the filename! */
                                                    }
                                            }
                                            index = index+2;
                                    }
                                    for (index=0; index<sizeof(lfndir->name2);)/* Repeat for second section of characters */
                                    {
                                            lfncharacter = (((WORD)lfndir->name2[index+1])<<8)+lfndir->name2[index];
                                            if (lfncharacter==0xFFFF)
                                            {
                                            }
                                            else
                                            {
                                                    ...
                                                    if (((lfncharacter>0)&&(lfncharacter<32))||(lfncharacter>127))
                                                    {
                                                            LFNokflag = LFNokflag|0x40;
                                                    }
                                            }
                                            index = index+2;
                                    }
                                    for (index=0; index<sizeof(lfndir->name3);)/* Repeat for third section of characters */
                                    {
                                            lfncharacter = (((WORD)lfndir->name3[index+1])<<8)+lfndir->name3[index];
                                            if (lfncharacter==0xFFFF)
                                            {
                                            }
                                            else
                                    ...
                                    if (lfndir->checksum!=LFNchecksum)
                                    {
                                            LFNokflag = LFNokflag|0x20;/* Different checksum! */
                                    }
                            }
                            else
                            {
                                    LFNokflag |= 0x80;/* Wrong sequence! */
                                    LFNstate = 255;
                            }
                    }
                    else
                    if (LFNokflag==0x01)
                    {
                            /* Apparently we have a valid long file name - but we have to confirm it's checksum matches this DOS filename! */
                            a = 0;
                for (index=0; index < DIR_NAMESIZE; index++)
                {
                            a = (((a & 1) << 7) | ((a & 0xfe) >> 1)) + dir->DIR_Name[index];
                }
                for (index=0; index < DIR_EXTENSION; index++)
                {
                            a = (((a & 1) << 7) | ((a & 0xfe) >> 1)) + dir->DIR_Extension[index];
                }
                            if (a!=LFNchecksum)
                            {
                                    LFNokflag = LFNokflag|0x10;/* Checksum does not match DOS! */
                            }
            }
#endif//LFNREAD
