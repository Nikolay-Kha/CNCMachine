...
/****************************************************************************
  Function:
    int USBHID_ReportDecriptor_Difference(void)

  Description:
    This function is used to detemine which is mouse or keyboard
  ...
  Return Values:
	NOT_SUPPORT_DEVICE	-- The deveice attached to USB Port is not supported.
	USB_HID_MOUSE		-- USB Port is attached by Mouse Device.
	USB_HID_KEYBOARD	-- USB Port is attached by KeyBoard Device.
...
{
	#ifdef DEBUG_MODE
	    UART2PrintString("\r\n ======================= Report Descriptor Difference ======================= \n\n");
		UART2PrintString("\r\nUsage: ");UART2PutHex(itemListPtrs.usageItemList[0].usage);
	#endif

        if(itemListPtrs.usageItemList[0].usage == 0x01)
		return USB_HID_KEYBOARD;
        else if(itemListPtrs.usageItemList[0].usage == 0x02)
		return USB_HID_MOUSE;
	else if(itemListPtrs.usageItemList[0].usage == 0x06)
		return USB_HID_KEYBOARD;
	else
		return NOT_SUPPORT_DEVICE;
}
